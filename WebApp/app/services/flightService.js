...
app.factory('flightService', ['$http', '$q', 'ngAuthSettings', '$rootScope', function ($http, $q, ngAuthSettings, $rootScope) {
    ...
    var _getFlightDailyReport = function (df, dt, from,to,regs,routes,no) {
        ...
        var deferred = $q.defer();
        $http.get(serviceBaseAPI + 'api/flight/daily?df=' + df + '&dt=' + dt + '&from=' + from+'&to='+to+'&regs='+regs+'&routes='+routes+'&no='+no).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
    ...
    var _updateFormVacation = function (entity) {
        var deferred = $q.defer();
        $http.post(serviceForms + 'api/vacation/update', entity).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
