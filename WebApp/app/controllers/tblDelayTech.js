'use strict';
app.controller('tblDelayTechController', ['$scope', '$location', 'personService', 'authService', 'biService', '$routeParams', '$rootScope', function ($scope, $location, personService, authService, biService, $routeParams, $rootScope) {
    ...
    $scope.bindMaster = function () {
        ...
        $scope.dg_master_ds = Enumerable.From($scope.categories)
            .Where(function (x) {
                return $scope.years.indexOf(x.Year) != -1 && $scope.selectedTypes.indexOf(x.Register) != -1;
            })
            .ToArray();
    ...
    $scope.dg_master_columns = [
        ...
        { dataField: 'Register', caption: 'Reg', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, minWidth: 125, fixed: true, fixedPosition: 'left', sortIndex: 2, sortOrder: 'asc' },
        {
    ...
    $scope.$on('InittblDelayTech', function (event, prms) {
        ...
        $scope.selectedCats = prms.initialCats;
        $scope.total = $scope.prms.total;
        $scope.categories = $scope.total.technicalRegisters;
