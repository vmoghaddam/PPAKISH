'use strict';
app.controller('studyFieldSelectController', ['$scope', '$location', 'authService', '$routeParams', '$rootScope', function ($scope, $location, authService, $routeParams, $rootScope) {
    ...
    $scope.popup_add_title = 'Study Fields';
    $scope.popup_add = {
    ...
    $scope.popup_add.toolbarItems[0].options.onClick = function (e) {
        ...
        $rootScope.$broadcast('onStudyFieldSelectHide', $scope.dg_selected);
        $scope.popup_add_visible = false;
    };
    ////////////////////////////
    $scope.bind = function () {
        if (!$scope.dg_ds) {
            $scope.dg_ds = {
                store: {
                    type: "odata",
                    url: $rootScope.serviceUrl + 'odata/options/'+'59',
                    key: "Id",
                    version: 4,
    ...
    $scope.$on('InitStudyFieldSelect', function (event, prms) {
