'use strict';
app.controller('journalController', ['$scope', '$location', '$routeParams', '$rootScope', 'generalService', 'authService', '$route', function ($scope, $location, $routeParams, $rootScope, generalService, authService, $route) {
    $scope.prms = $routeParams.prms;
    $scope.TypeId = $route.current.TypeId;
    $scope.caption = 'Journals';
    $scope.url = '';
    switch (Number($scope.TypeId)) {
        
        case 2:
            $scope.caption = 'Conferences';
            $scope.url = 'odata/base/conferences';
            break;
        case 1:
            $scope.caption = 'Journals';
            $scope.url = 'odata/base/journals';
            break;
        default:
            break;
    }
        ...
        onClick: function (e) {
            ...
            General.Confirm(Config.Text_DeleteConfirm, function (res) {
                if (res) {
                    ...
                    generalService.deleteJournal(dto).then(function (response) {
    ...
    $scope.btn_new = {
        ...
        onClick: function (e) {
            ...
            $rootScope.$broadcast('InitAddJournal', data);
        }
    ...
    $scope.dg_columns_conferences = [
        ...
        { dataField: 'Books', caption: 'Items', allowResizing: true, alignment: 'center', dataType: 'string', allowEditing: false, width: 110, },
    ];
    $scope.dg_columns_journals = [
        {
            cellTemplate: function (container, options) {
                $("<div style='text-align:center'/>")
                    .html(options.rowIndex + 1)
    ...
    switch (Number($scope.TypeId)) {
        ...
        case 2:
            ...
            $scope.dg_columns = $scope.dg_columns_conferences;
            break;
        case 1:
            //$scope.caption = 'Publishers';
            $scope.dg_columns = $scope.dg_columns_journals;
            break;
        default:
            break;
    }
    ...
    $scope.bind = function () {
        if (!$scope.dg_ds) {
            $scope.dg_ds = {
                ...
                sort: [{ getter: "Books", desc: true },'Title'],
    ...
    $scope.$on('onJournalSaved', function (event, prms) {

        $scope.doRefresh = true;
    });
    $scope.$on('onJournalHide', function (event, prms) {

        $scope.bind();

    });
    //////////////////////////////////////////
    $rootScope.$broadcast('JournalLoaded', null);
    ///end
}]);