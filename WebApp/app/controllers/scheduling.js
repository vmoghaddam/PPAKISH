...
app.controller('schedulingController', ['$scope', '$location', '$routeParams', '$rootScope', '$timeout', 'schedulingService', 'flightService', 'weatherService', 'aircraftService', 'authService', 'notificationService', '$route', '$window', function ($scope, $location, $routeParams, $rootScope, $timeout, schedulingService, flightService, weatherService, aircraftService, authService, notificationService, $route, $window) {
            ...
            try {
                ...
                $.each(response.resources, function (_i, _d) {
                    ...
                    $.each(flights, function (_j, _q) {
                        if (_q.FlightNumber == '7125')
                            console.log('FLT 7125',_q);
    ...
    $scope.checkContinuity2 = function (flights) {
        //2022-01-23
        var hasError = false;
        console.log('CHECK CONTINUITY');
        console.log(flights);
        var ordered = Enumerable.From(flights).OrderBy(function (x) { return new Date(x.offblock); }).ToArray();
        console.log(ordered);
        $.each(ordered, function (_i, _d) {
            if (_i >= 0 && _i < ordered.length - 1) {
                console.log(_i + '   ' + _d.ToAirport + '    ' + ordered[_i + 1].FromAirport);
                if (_d.ToAirport != ordered[_i + 1].FromAirport) { console.log('error'); hasError = true; }
            }
        });
        return hasError;
