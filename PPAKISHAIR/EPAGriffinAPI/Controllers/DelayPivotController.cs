...
namespace EPAGriffinAPI.Controllers
{
    ...
    public class DelayPivotController : ApiController
    {
        private EPAGRIFFINEntities _context;
        public DelayPivotController()
        {
            _context = new EPAGRIFFINEntities();
        }

        [Route("odata/pivot/delay/total")]
        public /*IEnumerable<RptFuelLeg>*/HttpResponseMessage GetDelayTotal(DataSourceLoadOptions loadOptions)
        {
            ...
            var orders = _context.RptDelayLegs;//from o in _context.RptFuelLegs
                                              //select new
                                              //{
                                              //    o.ID,
                                              //    o.PYear,
        ...
        public RptDelayLeg Get(int id)
        {
            return _context.RptDelayLegs.Find(id);
        }
        public int Post(RptDelayLeg cat)
        {
            _context.RptDelayLegs.Add(cat);
            return _context.SaveChanges();
        }
        public int Put(RptDelayLeg cat)
        {
            RptDelayLeg categ = _context.RptDelayLegs.Find(cat.ID);
            //categ.CategoryName = cat.CategoryName;
            return _context.SaveChanges();
        }
        public int Delete(int id)
        {
            RptDelayLeg cat = _context.RptDelayLegs.Find(id);
            _context.RptDelayLegs.Remove(cat);
            return _context.SaveChanges();
        }
    }
}
